...
import com.plaso.conf.ConferenceServiceAction;
...
//        if (jid.toBareJID().toLowerCase().contains("group") && packet instanceof Message 
//        		&& ((Message) packet).getType() == Message.Type.chat) {
//        	GroupServiceAction.getInstance().broadChatMsg((Message) packet);
//        	routed = true;
//        	return;
//        }
        
        if ((jid.toBareJID().toLowerCase().contains("live") || jid.toBareJID().toLowerCase().contains("class")) && packet instanceof Message 
        		&& ((Message) packet).getType() == Message.Type.chat) {
        	GroupServiceAction.getInstance().broadChatMsg((Message) packet);
        	routed = true;
        	return;
        ...
        if (jid.toBareJID().toLowerCase().contains("conf")  && packet instanceof Message 
        		&& ((Message) packet).getType() == Message.Type.chat) {
        	ConferenceServiceAction.getInstance().receiveConfCommand((Message) packet);
        	routed = true;
        	return;
        }
